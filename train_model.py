# -*- coding: utf-8 -*-
"""
分组BP神经网络训练脚本 (基于 xunlian1208.xlsx)
功能：
1. 读取 xunlian1208.xlsx 主数据文件。
2. 根据指定的策略将数据分组 (A, BCD, J, M, O)。
3. 使用 CUDA (如果可用) 训练模型。
4. 生成 model_registry.json 供 API 服务调用。
"""
...
ARTIFACTS_DIR = os.path.join(BASE_DIR, "bp_artifacts")
...
def train_group(group_name, target_types, df_all, output_dir):
    ...
    # 筛选数据
    df_train = df_all[df_all["产品类型"].isin(target_types)].copy()

    if len(df_train) < 10:
        print(f"    [跳过] 样本数不足 ({len(df_train)} 条)")
        return False

    print(f"    [信息] 样本数: {len(df_train)}")
    ...
    y_raw = df_train[[target_col]].values.astype(np.float32)
    ...
    loader = DataLoader(dataset, batch_size=min(64, len(df_train)), shuffle=True)
    ...
    model.eval()
    with torch.no_grad():
        p_scaled = model(torch.tensor(X_scaled, device=DEVICE)).cpu().numpy()
    p_raw = y_scaler.inverse_transform(p_scaled)

    r2 = r2_score(y_raw, p_raw)
    mse = mean_squared_error(y_raw, p_raw)
    mae = mean_absolute_error(y_raw, p_raw)
    print(f"    [结果] R2: {r2:.4f} | MSE: {mse:.4f} | MAE: {mae:.4f}")

    # 保存图表
    plt.figure(figsize=(6, 5))
    plt.scatter(y_raw, p_raw, alpha=0.6, label='Data')
    ...
    # 保存指标
    metrics = {"R2": r2, "MSE": mse, "MAE": mae, "Samples": len(df_train)}
...
def main():
    ...
    registry = {
        ...
        "description": "产品类型 -> 模型组映射 (基于 xunlian1208)"
    }
